...
IMAGE_ROOT = '../../tcdata/Test/'
FASTER_RCNN_CONFIG = './cascade_rcnn50_dcn_test/cascade_rcnn_r50_fpn_dconv_c3-c5_1x_coco.py'  # model configs
CKPT_PATH = './checkpoints/cascade_rcnn50_dcn_giou_pth'  # model checkpoints
RESULT_PATH = '../../prediction_result/'  # save the detection results
...
def main(cate_id, faster_rcnn_url, result_path):
    ...
    RESULTS = []
    with open(pt.join(IMAGE_ROOT, 'image_annos', 'person_bbox_test_B.json')) as f:
    ...
    for i, img_path in enumerate(img_paths):
        ...
        for idx in range(det_bboxes.shape[0]):
            ...
            RESULTS.append(item)
        #image_name=(img_path.split('/')[-1]).split('.')[0]+'_{}'.format(CATE[cate_id])+'.jpg'
        #show_after_nms(img_path, det_bboxes, det_labels, CATE[cate_id], pt.join('./visualize_results/cascade_results/2020_4_6_results/visible_full_results', image_name))
...
if __name__ == '__main__':
   ...
   for i in range(1,5):
    ...
    savepath = pt.join(RESULT_PATH, 'det_results.json')
